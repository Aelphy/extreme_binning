...
long long int RabinKarp::fingerprint(std::vector<char> data) {
    if (current_fingerprint_ == -1) {
        init_hash(data.data(), data.size());
    } else {
        update_hash(data.data(), data.size() - 1);
    }

    return current_fingerprint_;
}

long long int RabinKarp::compute_shifted_window(long long int s) {
...
void RabinKarp::init_hash(const char *data, int len) {
...
void RabinKarp::update_hash(const char *data, int new_index) {
